...
from src.util import valid_bibtex_types
from src.util.text import jaccard_similarity
from src.util.utils import Replacement, Similar, Upgrade


@register_command(name="dedup", description="Filtering duplicated entries based on Jaccard similarity by words.")
        ...
        for i, entry in enumerate(merged.entries):
            ...
            if deduped.entries:
                similarity, other_entry = max(
                    [(jaccard_similarity(value, other_entry[args.inspected_property]), other_entry) for other_entry in
                     deduped.entries],
                    key=lambda e: e[0]
                )
                if similarity < args.similarity_limit:
                    ...
                    if other_entry['ID'] != entry['ID']:
                        replacement = Replacement(
                            ...
                            Similar(entry, other_entry, args.inspected_property, similarity))
